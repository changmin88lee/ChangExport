    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        try
        {
            ...
            using var settings = new ExportSettingsForm(sheets, SheetSetService.ReadSets(document, configuration, sheets),
                ...
                configuration.SheetSourceOrder,
                ...
                (sets, assignments, sourceOrder) => { SheetSetService.ApplyAssignments(configuration, sets, assignments, sourceOrder); store.Save(configuration); });
