    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        var dialog = new TaskDialog("창Export " + ChangExport.App.ProductInfo.Version)
        {
            ...
            ExpandedContent =
                ...
                "DWG 2010으로 저장합니다. 원근·음영 뷰는 생략하고 이미지는 전체 크기·회전의 사각형으로 대체하여 나머지 도면과 세트를 저장합니다. 원근·음영 뷰만 있는 시트는 범위 사각형으로 자리를 유지합니다. 생략·대체 내역은 결과와 Manifest에서 확인하세요. 글자·치수·해치·잘림은 실제 출력 결과를 비교하세요. 커스텀 필터는 다음 단계이며 기존 CAD_LAYER와 Rule은 이번 출력에 적용하지 않습니다. " +
