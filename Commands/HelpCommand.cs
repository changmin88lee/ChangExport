    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        var dialog = new TaskDialog("창Export " + ChangExport.App.ProductInfo.Version)
        {
            ...
            ExpandedContent =
                ...
                "카테고리 아래 필터는 호스트와 로드된 링크의 유형 이름에 포함된 문자를 대소문자 구분 없이 비교하며 위에서 첫 일치만 적용합니다. 독립 복제 시트·뷰의 네이티브 뷰 필터로 링크까지 전파한 뒤 복구합니다. 링크 복합재료는 정확한 재료명과 충돌 없는 Revit 해치·색상으로 DWG 해치와 경계를 구분하며, 동일 표시 충돌·공유 뷰·그래픽 재지정 미지원 뷰는 제외 내역을 알립니다. 창Export가 객체 재지정을 유지하여 필터를 적용합니다. 필터 미반영 시 기본 출력과 경고를 남깁니다. 기존 CAD_LAYER와 구형 Rule은 연결하지 않습니다. " +
