...
internal static class FamilyRecognitionRegression
{
    ...
    private static void Synthetic(string output, Action<bool, string> check)
    {
        ...
        var names = new[] { "문 - D1-7001-평면", "문 - D1-V1-평면", "문 - D1-7002-평면", "문 - D2-7003-평면",
            "문 - D1추가-101-평면", "동명이형 - A-900-평면", "동명이형 - A-V2-평면", "동명이형 - A-V3-평면",
            "이름충돌 - X-901-평면", "철골 보 - B1-V1-평면", "점자블럭(원형) - 2EA-V1-평면",
            "알수없음 - Z-12-평면", "문 - D1-평면", "철골 보 - B1-301-평면", "상세그룹 A-9001-평면" };
        for (int index = 0; index < names.Length; index++)
        {
            var block = new BlockRecord(names[index]);
            double length = names[index].Contains("A-V2", StringComparison.Ordinal) ? 45 : 30;
            block.Entities.Add(new Line { StartPoint = XYZ.Zero, EndPoint = new XYZ(length, 0, 0) });
        ...
        var sources = new List<FamilyBlockSource> {
            ...
            new() { Identity = "host:A", Label = "동명이형 - A", Category = "문", NativeLabels = new() { "동명이형 - A" } },
            new() { Identity = "link:A", Label = "동명이형 - A", Category = "문", NativeLabels = new() { "동명이형 - A" } },
            new() { Identity = "door:X", Label = "이름충돌 - X", Category = "문", NativeLabels = new() { "이름충돌 - X" } },
            new() { Identity = "window:X", Label = "이름충돌 - X", Category = "창", NativeLabels = new() { "이름충돌 - X" } },
        ...
        check(result.FamilyBlockReferences == 9, "Automatic families and detail group match, recovering same-name fixed families by geometry");
        check(result.FamilyBlockDefinitions < result.FamilyBlockReferences, "Identical same-type geometry reuses a definition");
        check(result.FamilyBlockMatches.Any(m => m.NativeBlock.StartsWith("동명이형") && m.Status.Contains("형상 서명")),
            "Same-name host/link families advance to safe geometry comparison");
        check(result.FamilyBlockMatches.Any(m => m.NativeBlock.StartsWith("이름충돌") && m.Status.Contains("중복")),
            "Same-name candidates from different categories remain individual objects");
        ...
        var sameName = groupedDrawing.Entities.OfType<Insert>().Where(i => i.Block.Name.Contains("동명이형 - A", StringComparison.Ordinal)).ToArray();
        check(sameName.Length == 3 && sameName.Select(i => i.Block.Name).Distinct().Count() == 2,
            "Same-name identical geometry shares a block while a different shape keeps a separate definition");
    ...
    private static void Real(string output, string manifestPath, Action<bool, string> check)
    {
        ...
        JsonElement template = manifest.TryGetProperty("layerColors", out _) ? manifest
            : manifest.GetProperty("templates").EnumerateArray().First();
        var layers = JsonSerializer.Deserialize<List<LayerAppearance>>(template.GetProperty("layerColors").GetRawText())!;
        var wide = JsonSerializer.Deserialize<List<WideLineLayer>>(template.GetProperty("wideLineLayers").GetRawText())!;
        ...
        var processor = new ManagedDwgProcessor();
        var sheets = paths.Where(p => Path.GetFileName(p) == "sheet.dwg"
                && Path.GetFileName(Path.GetDirectoryName(p))?.StartsWith("filtered_", StringComparison.OrdinalIgnoreCase) == true)
            .OrderBy(p => p).ToArray();
        ...
        check(inserts.Any(i => i.Block.Name.Contains("외여닫이문") || i.Block.Name.Contains("세대현관문") || i.Block.Name.Contains("점검구_")),
            "Actual swing and entrance doors become blocks");
        check(inserts.Any(i => i.Block.Name.Contains("자동문")), "Actual automatic doors become blocks");
        check(inserts.Any(i => i.Block.Name.Contains("점자블럭_원형_개수_면기반_")), "Actual face-based tactile families become blocks");
