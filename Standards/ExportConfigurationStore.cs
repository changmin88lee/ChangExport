...
public sealed class ExportConfigurationStore
{
    ...
    public RevitExportConfiguration Load()
    {
        ...
        if (config.SchemaVersion is not (1 or 2 or 3 or 4 or 5 or 6 or 7) || config.Setups is null || config.SheetSets is null || config.WideLineKeyword == null)
        ...
        if (config.SchemaVersion == 6) config.SchemaVersion = 7;
        ...
        config.SheetSourceOrder = (config.SheetSourceOrder ?? new()).Where(key => !string.IsNullOrWhiteSpace(key))
