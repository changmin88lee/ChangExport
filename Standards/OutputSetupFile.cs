...
public sealed class OutputSetupFile
{
    ...
    public void Validate()
    {
        ...
        if (Layers.Any(r => r == null || string.IsNullOrWhiteSpace(r.Category) || !ViewLayerScope.IsValid(r.ViewScope, allowLegacy: true)
            ...
            || (MaterialRules ?? new()).Any(r => r == null || !ViewLayerScope.IsValid(r.ViewScope, allowLegacy: true))
        ...
        var issues = RevitLayerMappingService.Validate(full).ToList();
        issues.AddRange(RevitLayerMappingService.ValidateMaterialRules(MaterialRules ?? new()));
        issues.AddRange(RevitLayerMappingService.ValidateCombined(full, MaterialRules ?? new()));
